...
(function(orionWidow) {
    ...
    $.OrionCart = function (opts) {
        ...
        if (vtexjs && vtexjs.checkout && vtexjs.checkout.orderForm) {
            ...
            totalizersConfig(vtexjs.checkout.orderForm.items, vtexjs.checkout.orderForm.totalizers);
            fixHeightConfig();
        ...
        // Buscando o orderForm manualmente
            ...
            vtexjs.checkout.getOrderForm().done(function(orderForm) {
                ...
                totalizersConfig(orderForm.items, orderForm.totalizers);
                fixHeightConfig();
        ...
        function renderItems (items) {
            ...
            items.forEach(function (item, index, array) {
                let imgFormated = thumbConfig(item); // formatando a thumb
                let newBlock = '<div class="cart-item">';
                newBlock += '<div class="cart-item-image"><img src="'+imgFormated+'" /></div><div class="cart-item-info">';
                newBlock += '<div class="cart-item-info--name"><span class="name">'+item.name+'</span><span class="price">R$'+ orionFormatPrice(item.price / 100, 2, ",", ".")+'</span></div><div class="cart-item-info--price--buttons">';
                newBlock += '<span class="cart-item-text-qtt">Quantidade</span><ul class="list-buttons-cart-item" data-of-index="'+index+'">';
                newBlock += '<li><a class="cart-btns-update cart-btn-minus">'+pubIcons.minus+'</a></li>';
                newBlock += '<li class="item-total"><a class="cart-btn-total" data-prod-qtt="'+item.quantity+'">'+item.quantity+'</a>'+pubIcons.loader+'</li>';
                newBlock += '<li><a class="cart-btns-update cart-btn-more">'+pubIcons.more+'</a></li>';
                newBlock += '<li class="item-remove"><a class="cart-btn-remove">'+pubIcons.remove+'</a>'+pubIcons.loader+'</li>';
                newBlock += '</ul></div></div></div>';

                // Agrupando
                listItens = listItens + newBlock;
            });
        ...
        function totalizersConfig (items, totalizers) { 
            ...
            $(".totalizers-totals-number").text("R$ " + priceItems);
        }
